...
This document describes the implementation of the service abstraction layer that decouples the Papillon frontend from Tauri IPC, enabling support for pure WebAssembly environments while maintaining backward compatibility with the Tauri desktop app.
...
```
apps/papillon/frontend/src/service/
...
### 1. PapillonService Trait (`mod.rs`)
...
- `use_papillon_service()`: Get service from component context
- `init_papillon_service()`: Initialize service on app startup
...
`get_papillon_service()` dispatches to correct implementation:
...
Exports available:
- `PapillonService` trait
- `use_papillon_service()` hook
- `init_papillon_service()` factory
...
impl PapillonService for HttpService {
    async fn get_global_templates(&self) -> Result<Vec<Template>, String> {
...
1. `apps/papillon/frontend/src/service/mod.rs` (177 lines)
   - PapillonService trait definition
   - Factory function and helper types

2. `apps/papillon/frontend/src/service/tauri_service.rs` (198 lines)
   - TauriService implementation
   - Direct bridge delegation

3. `apps/papillon/frontend/src/service/web_service.rs` (194 lines)
   - WebService stub implementation
   - Placeholder for IndexedDB

4. `apps/papillon/frontend/src/service/hooks.rs` (78 lines)
   - Leptos integration hooks
   - Context management

5. `apps/papillon/frontend/MIGRATION_GUIDE.md`
...
1. `apps/papillon/frontend/src/lib.rs`
   - Added: `pub mod service;`

2. `apps/papillon/frontend/Cargo.toml`
