...
**Scope:** Pure refactor of existing code. Create a `PapillonService` trait with two implementations:
...
#### 1. Core Service Module (`apps/papillon/frontend/src/service/`)

**`mod.rs` (177 lines)**
- `PapillonService` trait with 27 async methods
- Comprehensive operation coverage: templates, profiles, identity, registry, orchestrator, scenarios, agent profiles
- `get_papillon_service()` factory function
...
**`hooks.rs` (78 lines)**
- `use_papillon_service()` hook for components
- `init_papillon_service()` factory function
...
let service = get_papillon_service().await;
```
...
1. `apps/papillon/frontend/src/lib.rs` (+1 line)
   - Add: `pub mod service;`

2. `apps/papillon/frontend/Cargo.toml` (+1 line)
   - Add: `async-trait = "0.1"`
...
- [x] Create PapillonService trait ✓
