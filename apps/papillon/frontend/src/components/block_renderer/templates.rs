...
/// SearchResultsPage / SearchAction template — renders search results.
///
/// Handles multiple payload shapes from agents:
/// 1. **Ideal**: `mainEntity.itemListElement` — proper schema.org ItemList
/// 2. **Mapped**: flat `name`/`description`/`url` — from `response_mapping` agents
/// 3. **Legacy**: bare `description` string — from single-field `response_jsonpath`
///
/// A template must NEVER return an empty view. If it claims a type, it renders.
...
impl SearchTemplate {
    ...
    /// Try 1: Extract items from `mainEntity.itemListElement` (ideal schema.org).
    ...
    /// Render a list of structured search result items.
    fn render_item_list(items: Vec<Value>) -> AnyView {
        let rendered = items
            .into_iter()
            .map(|item| {
    ...
    /// Try 2: Render flat schema.org properties (name, description, url, image, source).
    /// Produced by agents with `response_mapping`.
    fn render_flat_result(content: &Value) -> AnyView {
        let name = content
            .get("name")
            .or_else(|| content.get("headline"))
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string();
        let description = Self::item_description(content);
        let url = content
            .get("url")
            .and_then(|v| v.as_str())
            .unwrap_or("")
        ...
        view! {
            <div class="typed-search-results">
                <div class="typed-search-item">
                    {has_name.then(|| view! {
                        <span class="typed-search-title">{name.clone()}</span>
                    })}
                    {has_url.then(|| view! {
                        <span class="typed-search-url">{url.clone()}</span>
                    })}
                    <span class="typed-search-snippet">{description}</span>
                    {has_source.then(|| view! {
                        <span class="typed-search-source">{source.clone()}</span>
                    })}
                    {has_image.then(|| view! {
                        <span class="typed-search-image">{image.clone()}</span>
                    })}
                </div>
            </div>
        }
    ...
    /// Try 3: Find any displayable text in the content as a last resort.
    fn render_any_text(content: &Value) -> AnyView {
        // Collect all string values (skip @-prefixed JSON-LD metadata)
        let text = content
            .as_object()
            .map(|obj| {
                obj.iter()
        ...
        if text.is_empty() {
            // Absolute fallback — show the raw JSON
            let raw = serde_json::to_string_pretty(content).unwrap_or_default();
            view! {
                <div class="typed-search-results">
                    <pre class="typed-raw-json">{raw}</pre>
                </div>
            }
            .into_any()
        } else {
            view! {
                <div class="typed-search-results">
                    <div class="typed-search-item">
                        <span class="typed-search-snippet">{text}</span>
                    </div>
                </div>
            }
            .into_any()
...
impl BlockRenderer for SearchTemplate {
    fn render(&self, content: &Value) -> AnyView {
        // Try 1: Proper ItemList structure (ideal schema.org)
        let items = Self::extract_items(content);
        if !items.is_empty() {
            return Self::render_item_list(items);
        }

        // Try 2: Flat schema.org properties (from response_mapping agents)
        let has_name = content.get("name").and_then(|v| v.as_str()).is_some()
            || content.get("headline").and_then(|v| v.as_str()).is_some();
        let has_desc = content
            .get("description")
            .and_then(|v| v.as_str())
            .is_some();
        if has_name || has_desc {
            return Self::render_flat_result(content);
        }

        // Try 3: Any text content at all
        Self::render_any_text(content)
    }
