...
///
/// Separate from the main `papillion.db` to enable:
...
impl ProfilesDatabase {
    ...
    pub fn open(path: &Path) -> Result<Self, PapillionError> {
        let conn = Connection::open(path)
            .map_err(|e| PapillionError::from(format!("profiles_db open: {e}")))?;
    ...
    fn migrate(&self) -> Result<(), PapillionError> {
        ...
        .map_err(|e| PapillionError::from(format!("profiles_db migrate: {e}")))?;
    ...
    pub fn create_profile(
        ...
        name: &str,
        seed_b64: &str,
    ) -> Result<ProfileMetadata, PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
        ...
        .map_err(|e| PapillionError::from(format!("profiles_db create: {e}")))?;
    ...
    /// List all profiles.
    pub fn list_profiles(&self) -> Result<Vec<ProfileMetadata>, PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
        ...
        let mut stmt = conn
            ...
            .map_err(|e| PapillionError::from(format!("profiles_db prepare: {e}")))?;
        ...
        for row in rows {
            profiles.push(row.map_err(|e| PapillionError::from(format!("profiles_db row: {e}")))?);
        }
        Ok(profiles)
    }

    /// Get the active profile (marked with active=1).
    pub fn get_active_profile(&self) -> Result<Option<ProfileMetadata>, PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
        ...
        match rows.next() {
            ...
            Some(Err(e)) => Err(PapillionError::from(format!("profiles_db row: {e}"))),
            None => Ok(None),
        }
    }

    /// Get a profile by ID.
    pub fn get_profile(&self, id: &str) -> Result<Option<ProfileMetadata>, PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
    ...
    pub fn get_profile_seed(&self, id: &str) -> Result<Option<String>, PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
    ...
    pub fn switch_profile(&self, id: &str) -> Result<(), PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
        ...
        conn.execute("BEGIN TRANSACTION", [])
            .map_err(|e| PapillionError::from(format!("profiles_db begin: {e}")))?;
        ...
        if exists == 0 {
            conn.execute("ROLLBACK", [])
                .map_err(|e| PapillionError::from(format!("profiles_db rollback: {e}")))?;
            return Err(PapillionError::from(format!("Profile {id} not found")));
        }
        ...
        // Commit transaction
        conn.execute("COMMIT", [])
            .map_err(|e| PapillionError::from(format!("profiles_db commit: {e}")))?;

        Ok(())
    }

    /// Rename a profile.
    pub fn rename_profile(&self, id: &str, new_name: &str) -> Result<(), PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
        ...
        .map_err(|e| PapillionError::from(format!("profiles_db rename: {e}")))?;
    ...
    pub fn delete_profile(&self, id: &str) -> Result<(), PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
        ...
        let count: i64 = conn
            ...
            .map_err(|e| PapillionError::from(format!("profiles_db count: {e}")))?;
        ...
        conn.execute("DELETE FROM profiles WHERE id = ?1", params![id])
            .map_err(|e| PapillionError::from(format!("profiles_db delete: {e}")))?;

        Ok(())
    }

    /// Update last_used timestamp for a profile.
    pub fn mark_last_used(&self, id: &str) -> Result<(), PapillionError> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| PapillionError::from(e.to_string()))?;
...
mod tests {
    ...
    impl ProfilesDatabase {
        ...
        fn open_memory() -> Result<Self, PapillionError> {
