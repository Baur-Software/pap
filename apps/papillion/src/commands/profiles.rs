...
) -> Result<Vec<ProfileMetadata>, PapillionError> {
...
pub async fn switch_profile(
    ...
    let profile = state
        ...
        .ok_or_else(|| PapillionError::from(format!("Profile {profile_id} not found")))?;

    // Load seed from profiles database
    let seed_b64 = state
        .profiles_db
        .get_profile_seed(&profile_id)?
        .ok_or_else(|| PapillionError::from(format!("No seed found for profile {profile_id}")))?;

    // Decode seed
    let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(&seed_b64)
        .map_err(|e| PapillionError::from(format!("Failed to decode seed: {e}")))?;

    let seed: [u8; 32] = bytes
        .try_into()
        .map_err(|_| PapillionError::from("Invalid seed length"))?;

    // Recreate keypair and signer
    let keypair = PrincipalKeypair::from_bytes(&seed)
        .map_err(|e| PapillionError::from(format!("Failed to recreate keypair: {e}")))?;
...
pub async fn rename_profile(
    ...
    state
        .profiles_db
        .get_profile(&profile_id)?
        .ok_or_else(|| PapillionError::from(format!("Profile {profile_id} not found after rename")))
}
...
pub async fn import_profile_seed(
    ...
    let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(&seed_b64)
        .map_err(|e| PapillionError::from(format!("Invalid seed encoding: {e}")))?;
    ...
    let seed: [u8; 32] = bytes
        .try_into()
        .map_err(|_| PapillionError::from("Failed to convert seed bytes"))?;

    PrincipalKeypair::from_bytes(&seed)
        .map_err(|e| PapillionError::from(format!("Invalid seed: {e}")))?;
