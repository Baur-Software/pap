...
pub(crate) fn topological_sort(pipeline: &PipelineInfo) -> Result<Vec<String>, PapillionError> {
    ...
    if order.len() != pipeline.nodes.len() {
        return Err(PapillionError::from("Pipeline contains a cycle"));
    }
    Ok(order)
}
...
pub async fn run_pipeline(
    ...
    // Build node lookup
    let nodes: HashMap<&str, &papillion_shared::PipelineNodeInfo> =
    ...
    for (step_idx, node_id) in order.iter().enumerate() {
        let node = nodes.get(node_id.as_str()).ok_or_else(|| {
            PapillionError::from(format!("Node {} not found in pipeline", node_id))
        })?;
