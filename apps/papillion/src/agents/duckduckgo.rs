...
impl AgentHandler for DuckDuckGoAgent {
    ...
    fn handle_disclosure(
        ...
        // Extract query from disclosures
    ...
    fn execute(&self, session_id: &str) -> Result<serde_json::Value, TransportError> {
        ...
        // Blocking HTTP call to DuckDuckGo
    ...
    fn co_sign_receipt(
        ...
        // Find the session key for this receipt's session
        // If session already closed, use a fresh ephemeral key
        let sessions = self.sessions.lock().unwrap();
        let key = sessions
            .values()
            .next()
