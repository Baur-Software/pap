...
//!   1. **Raw `ed25519_dalek`** — duplicates the library's own internal vectors
//!      as a version-pinning canary. If these break after a dependency bump, the
//!      crate is producing non-standard signatures.
//!   2. **`PrincipalKeypair` wrapper** — proves that `pap-did`'s signing/verify
//!      surface faithfully delegates to the raw layer without munging bytes.
...
// ────────────────────────────────────────────────────────────────────
const RFC8032_1_SECRET: [u8; 32] = [
    0x9d, 0x61, 0xb1, 0x9d, 0xef, 0xfd, 0x5a, 0x60,
    0xba, 0x84, 0x4a, 0xf4, 0x92, 0xec, 0x2c, 0xc4,
    0x44, 0x49, 0xc5, 0x69, 0x7b, 0x32, 0x69, 0x19,
    0x70, 0x3b, 0xac, 0x03, 0x1c, 0xae, 0x7f, 0x60,
];
const RFC8032_1_PUBLIC: [u8; 32] = [
    0xd7, 0x5a, 0x98, 0x01, 0x82, 0xb1, 0x0a, 0xb7,
    0xd5, 0x4b, 0xfe, 0xd3, 0xc9, 0x64, 0x07, 0x3a,
    0x0e, 0xe1, 0x72, 0xf3, 0xda, 0xa6, 0x23, 0x25,
    0xaf, 0x02, 0x1a, 0x68, 0xf7, 0x07, 0x51, 0x1a,
];
const RFC8032_1_MESSAGE: &[u8] = &[];
const RFC8032_1_SIGNATURE: [u8; 64] = [
...
// ────────────────────────────────────────────────────────────────────
const RFC8032_2_SECRET: [u8; 32] = [
    0x4c, 0xcd, 0x08, 0x9b, 0x28, 0xff, 0x96, 0xda,
    0x9d, 0xb6, 0xc3, 0x46, 0xec, 0x11, 0x4e, 0x0f,
    0x5b, 0x8a, 0x31, 0x9f, 0x35, 0xab, 0xa6, 0x24,
    0xda, 0x8c, 0xf6, 0xed, 0x4f, 0xb8, 0xa6, 0xfb,
];
const RFC8032_2_PUBLIC: [u8; 32] = [
    0x3d, 0x40, 0x17, 0xc3, 0xe8, 0x43, 0x89, 0x5a,
    0x92, 0xb7, 0x0a, 0xa7, 0x4d, 0x1b, 0x7e, 0xbc,
    0x9c, 0x98, 0x2c, 0xcf, 0x2e, 0xc4, 0x96, 0x8c,
    0xc0, 0xcd, 0x55, 0xf1, 0x2a, 0xf4, 0x66, 0x0c,
];
const RFC8032_2_MESSAGE: &[u8] = &[0x72];
const RFC8032_2_SIGNATURE: [u8; 64] = [
...
// ────────────────────────────────────────────────────────────────────
const RFC8032_3_SECRET: [u8; 32] = [
    0xc5, 0xaa, 0x8d, 0xf4, 0x3f, 0x9f, 0x83, 0x7b,
    0xed, 0xb7, 0x44, 0x2f, 0x31, 0xdc, 0xb7, 0xb1,
    0x66, 0xd3, 0x85, 0x35, 0x07, 0x6f, 0x09, 0x4b,
    0x85, 0xce, 0x3a, 0x2e, 0x0b, 0x44, 0x58, 0xf7,
];
const RFC8032_3_PUBLIC: [u8; 32] = [
    0xfc, 0x51, 0xcd, 0x8e, 0x62, 0x18, 0xa1, 0xa3,
    0x8d, 0xa4, 0x7e, 0xd0, 0x02, 0x30, 0xf0, 0x58,
    0x08, 0x16, 0xed, 0x13, 0xba, 0x33, 0x03, 0xac,
    0x5d, 0xeb, 0x91, 0x15, 0x48, 0x90, 0x80, 0x25,
];
const RFC8032_3_MESSAGE: &[u8] = &[0xaf, 0x82];
const RFC8032_3_SIGNATURE: [u8; 64] = [
...
// ────────────────────────────────────────────────────────────────────
// RFC 8032 Section 7.1 — TEST SHA(abc) (64-byte message)
// ────────────────────────────────────────────────────────────────────
const RFC8032_SHA_ABC_SECRET: [u8; 32] = [
    0x83, 0x3f, 0xe6, 0x24, 0x09, 0x23, 0x7b, 0x9d,
    0x62, 0xec, 0x77, 0x58, 0x75, 0x20, 0x91, 0x1e,
    0x9a, 0x75, 0x9c, 0xec, 0x1d, 0x19, 0x75, 0x5b,
    0x7d, 0xa9, 0x01, 0xb9, 0x6d, 0xca, 0x3d, 0x42,
];
const RFC8032_SHA_ABC_PUBLIC: [u8; 32] = [
...
const RFC8032_SHA_ABC_MESSAGE: [u8; 64] = [
...
const RFC8032_SHA_ABC_SIGNATURE: [u8; 64] = [
...
// ────────────────────────────────────────────────────────────────────
// RFC 8032 Section 7.1 — TEST 1023-byte message
// ────────────────────────────────────────────────────────────────────
const RFC8032_1024_SECRET: [u8; 32] = [
...
const RFC8032_1024_PUBLIC: [u8; 32] = [
...
#[rustfmt::skip]
const RFC8032_1024_MESSAGE: [u8; 1023] = [
...
const RFC8032_1024_SIGNATURE: [u8; 64] = [
...
// ════════════════════════════════════════════════════════════════════
// Layer 1 — raw ed25519_dalek (version-pinning canary)
// ════════════════════════════════════════════════════════════════════

#[test]
fn rfc8032_test1_raw_ed25519() {
    // RFC 8032 Section 7.1 — TEST 1 (empty message)
    // Duplicates ed25519-dalek's own vectors as a version-pinning canary.
    let sk = SigningKey::from_bytes(&RFC8032_1_SECRET);
    assert_eq!(sk.verifying_key().to_bytes(), RFC8032_1_PUBLIC);
    let sig = sk.sign(RFC8032_1_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_1_SIGNATURE);
}

#[test]
fn rfc8032_test2_raw_ed25519() {
    // RFC 8032 Section 7.1 — TEST 2 (1-byte message)
    let sk = SigningKey::from_bytes(&RFC8032_2_SECRET);
    assert_eq!(sk.verifying_key().to_bytes(), RFC8032_2_PUBLIC);
    let sig = sk.sign(RFC8032_2_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_2_SIGNATURE);
}

#[test]
fn rfc8032_test3_raw_ed25519() {
    // RFC 8032 Section 7.1 — TEST 3 (2-byte message)
    let sk = SigningKey::from_bytes(&RFC8032_3_SECRET);
    assert_eq!(sk.verifying_key().to_bytes(), RFC8032_3_PUBLIC);
    let sig = sk.sign(RFC8032_3_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_3_SIGNATURE);
}

#[test]
fn rfc8032_test_sha_abc_raw_ed25519() {
    // RFC 8032 Section 7.1 — TEST SHA(abc) (64-byte message)
    let sk = SigningKey::from_bytes(&RFC8032_SHA_ABC_SECRET);
    assert_eq!(sk.verifying_key().to_bytes(), RFC8032_SHA_ABC_PUBLIC);
    let sig = sk.sign(&RFC8032_SHA_ABC_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_SHA_ABC_SIGNATURE);
}

#[test]
fn rfc8032_test_1024_raw_ed25519() {
    // RFC 8032 Section 7.1 — TEST 1023-byte message
    let sk = SigningKey::from_bytes(&RFC8032_1024_SECRET);
    assert_eq!(sk.verifying_key().to_bytes(), RFC8032_1024_PUBLIC);
    let sig = sk.sign(&RFC8032_1024_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_1024_SIGNATURE);
}

// ════════════════════════════════════════════════════════════════════
// Layer 2 — PrincipalKeypair wrapper (pap-did delegation check)
// ════════════════════════════════════════════════════════════════════

#[test]
fn rfc8032_test1_principal_chain() {
    // RFC 8032 Section 7.1 — TEST 1 via PrincipalKeypair
    let kp = PrincipalKeypair::from_bytes(&RFC8032_1_SECRET).unwrap();
    assert_eq!(kp.public_key_bytes(), RFC8032_1_PUBLIC);
    let sig = kp.sign(RFC8032_1_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_1_SIGNATURE);
    assert!(kp.verify(RFC8032_1_MESSAGE, &sig).is_ok());
}

#[test]
fn rfc8032_test2_principal_chain() {
    // RFC 8032 Section 7.1 — TEST 2 via PrincipalKeypair
    let kp = PrincipalKeypair::from_bytes(&RFC8032_2_SECRET).unwrap();
    assert_eq!(kp.public_key_bytes(), RFC8032_2_PUBLIC);
    let sig = kp.sign(RFC8032_2_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_2_SIGNATURE);
    assert!(kp.verify(RFC8032_2_MESSAGE, &sig).is_ok());
}

#[test]
fn rfc8032_test3_principal_chain() {
    // RFC 8032 Section 7.1 — TEST 3 via PrincipalKeypair
    let kp = PrincipalKeypair::from_bytes(&RFC8032_3_SECRET).unwrap();
    assert_eq!(kp.public_key_bytes(), RFC8032_3_PUBLIC);
    let sig = kp.sign(RFC8032_3_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_3_SIGNATURE);
    assert!(kp.verify(RFC8032_3_MESSAGE, &sig).is_ok());
}

#[test]
fn rfc8032_test_sha_abc_principal_chain() {
    // RFC 8032 Section 7.1 — TEST SHA(abc) via PrincipalKeypair
    let kp = PrincipalKeypair::from_bytes(&RFC8032_SHA_ABC_SECRET).unwrap();
    assert_eq!(kp.public_key_bytes(), RFC8032_SHA_ABC_PUBLIC);
    let sig = kp.sign(&RFC8032_SHA_ABC_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_SHA_ABC_SIGNATURE);
    assert!(kp.verify(&RFC8032_SHA_ABC_MESSAGE, &sig).is_ok());
}

#[test]
fn rfc8032_test_1024_principal_chain() {
    // RFC 8032 Section 7.1 — TEST 1023-byte message via PrincipalKeypair
    let kp = PrincipalKeypair::from_bytes(&RFC8032_1024_SECRET).unwrap();
    assert_eq!(kp.public_key_bytes(), RFC8032_1024_PUBLIC);
    let sig = kp.sign(&RFC8032_1024_MESSAGE);
    assert_eq!(sig.to_bytes(), RFC8032_1024_SIGNATURE);
