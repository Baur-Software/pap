...
impl<T> SessionStore<T> {
    ...
    pub fn insert(&self, session_id: String, data: T) -> Result<String, TransportError> {
        ...
        if map.len() >= MAX_SESSIONS {
            return Err(TransportError::ServerError("session limit reached".into()));
        }
        map.insert(
            session_id,
            Entry {
...
mod tests {
    ...
    fn insert_returns_did_string() {
        let store: SessionStore<u32> = make_store();
        let did = store.insert("s1".into(), 1).expect("insert should succeed");
    ...
    fn with_mut_triggers_reap() {
        ...
        assert!(
            result.is_ok(),
            "with_mut near cap should succeed: {result:?}"
        );
    }
    ...
    fn signing_key_is_stable() {
        ...
        assert!(
            second.is_some(),
            "second signing_key call should return Some"
        );
    }
}
