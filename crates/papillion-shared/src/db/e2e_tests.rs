...
#[cfg(feature = "native")]
mod e2e_scenarios {
    use crate::db::{Episode, AgentProfile, DatabaseOps};
    use chrono::Utc;
    ...
    fn e2e_scenario_complete_session() {
        ...
        for i in 0..10 {
            let episode = Episode {
                ...
                .into(),
                agent_did_hash: format!("{}-agent-{}",
                    match i % 3 { 0 => "search", 1 => "booking", _ => "payment" },
                    1
                ).into(),
        ...
        let search_episodes = db.list_episodes(Some("search"), None, 100, None)
            .expect("Failed to filter by action_type");
        assert_eq!(search_episodes.len(), 4, "Expected 4 search episodes (indices 0, 3, 6, 9)");

        let booking_episodes = db.list_episodes(Some("booking"), None, 100, None)
            .expect("Failed to filter by action_type");
        assert_eq!(booking_episodes.len(), 3, "Expected 3 booking episodes (indices 1, 4, 7)");

        // Phase 5: Search functionality
        println!("Phase 5: Testing search...");

        let tokyo_results = db.search_text("Tokyo", 100)
            .expect("Failed to search for 'Tokyo'");
        assert!(!tokyo_results.is_empty(), "Expected search results for 'Tokyo'");

        let schema_results = db.search_by_schema_type("FlightReservation", 100)
            .expect("Failed to search by schema type");
        assert_eq!(schema_results.len(), 3, "Expected 3 FlightReservation episodes");
        ...
        let query = db.get_setting("last_search_query")
    ...
    fn e2e_scenario_pagination() {
        ...
        for id in &page_1_ids {
            assert!(!page_2_ids.contains(id), "Page 2 should not contain episodes from page 1");
        }
    ...
    fn e2e_scenario_concurrent_updates() {
        ...
        // Verify final state
        let final_profile = db.get_agent_profile("agent-1")
            .expect("Failed to get profile")
            .expect("Profile should exist");

        assert_eq!(final_profile.success_rate, 0.88, "Profile should reflect final update");
        assert_eq!(final_profile.episode_count, 40, "Profile should reflect final episode count");
    ...
    fn e2e_scenario_wasm_complete_workflow() {
        ...
        let db = IndexedDbDatabase::new_with_persistence("e2e-test")
        ...
        // Verify retrieval
        let episodes = db.list_episodes(None, None, 10, None)
...
#[cfg(feature = "native")]
mod benchmarks {
    use crate::db::{Episode, DatabaseOps};
    use chrono::Utc;
    ...
    fn benchmark_episode_insertion() {
        ...
        println!("  Average: {:.2}ms per insert", elapsed.as_millis() as f64 / 1000.0);
    ...
    fn benchmark_query_performance() {
        ...
        let start = Instant::now();
        for _ in 0..100 {
            let _results = db.list_episodes(Some("search"), None, 50, None)
                .expect("Failed to query");
        }
        let elapsed = start.elapsed();

        println!("Executed 100 filtered queries on 1000 episodes in {:?}", elapsed);
        println!("  Average: {:.2}ms per query", elapsed.as_millis() as f64 / 100.0);
